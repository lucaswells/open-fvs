     ...
     >                   IRECNT,IREAD,JOSTND,IRC)
      IMPLICIT NONE
C----------
C BASE $Id: algexp.f 3856 2021-12-08 03:07:46Z lancedavid $
C----------
C
C     CALLED FROM EVUSRV AND HVIN (AND PERHAPS OTHER PLACES).
C     READS USER DEFINED VARIABLES.
C
C     EVENT MONITOR - N.L. CROOKSTON  - APRIL 1987
...
C
      ...
      RECLEN=LEN(RECORD)
C
C     LOAD VARIABLE WITH SPECIAL CHARACTERS THAT ARE INVALID FOR USE
C     IN USER DEFINED VARIABLE NAMES. THE 2 VALID SPECIAL CHARACTERS
C     NOT PRESENT IN STRING BELOW ARE _ (UNDERSCORE) AND $ (DOLLAR SIGN)
C
      DATA CHRSPCL /'^!"#%&''()*+,-./:;<=>?@[\]^`{|}~'/
C
...
C
   ...
   20    CONTINUE
C
C        SCAN USER DEFINED VARIABLE NAME FOR INVALID SPECIAL CHARACTERS
C        IN THE FIRST 8 POSITIONS, REJECT AS INVALID NAME IF FOUND.
C
         DO I=1,8
           DO J=1,31
